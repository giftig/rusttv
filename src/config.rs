...
fn default_allowed_exts() -> Vec<String> {
    vec!["avi", "m4v", "ass", "3gp", "mkv", "mp4", "srt"].into_iter().map(String::from).collect()
}

// Remote defaults
fn default_host() -> String { "giftig-pi".to_string() }
fn default_port() -> usize { 22 }
...
fn sub_vars(line: &str) -> String {
    ...
    let sub_one = |s: &str, caps: Captures| -> String {
        ...
        let value = env::var(group.as_str()).ok().unwrap_or_else(|| "".to_string());
...
fn read_raw() -> String {
    ...
    panic!("Couldn't find config in any of the following locations: {:?}", config_files);
}
